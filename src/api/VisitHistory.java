...
public class VisitHistory extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// not allow user A access user B's data
		if (!session.getAttribute("user").equals(request.getParameter("user_id"))) {
			response.setStatus(403);
			return;
		}
		...
		try {
			...
			if (request.getParameterMap().containsKey("user_id")) {
				...
				Set<String> ids = connection.getVisitedRestaurants(userId);
				array = new JSONArray();
				for (String id : ids) {
					// JSONObject obj =
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (input.has("user_id") && input.has("visited")) {
				...
				List<String> unvisitedRestaurants = new ArrayList<>();
				for (int i = 0; i < array.length(); i++) {
					String businessId = (String) array.get(i);
					unvisitedRestaurants.add(businessId);
				}
				connection.unsetVisitedRestaurants(userId, unvisitedRestaurants);
				RpcParser.writeOutput(response, new JSONObject().put("status", "OK"));
			} else {
				RpcParser.writeOutput(response, new JSONObject().put("status", "InvalidParameter"));
			}
